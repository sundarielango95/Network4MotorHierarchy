# -*- coding: utf-8 -*-
"""
Created on Wed Aug 31 13:05:39 2022
...
# dd = uk[:,0,:2]
# plt.figure()
# plt.scatter(uk[:,1,0],uk[:,1,1],marker='*',color='r')
...
batch = 1
lr = 0.001#0.00025#0.00000025 #0.001 - mse loss
...
# torch.save(lppc_model.state_dict(), "models/rppc_lppc/lppc/lppc_v1_1_"+str(epoch)+"_epochs")
## uncomment for V2 mode of training
# lppc_model.fc_1x[0].weight.requires_grad = False
# lppc_model.fc_1x[0].bias.requires_grad = False
# lppc_model.fc_2x[0].weight.requires_grad = False
# lppc_model.fc_2x[0].bias.requires_grad = False
# lppc_model.fc_3x[0].weight.requires_grad = False
# lppc_model.fc_3x[0].bias.requires_grad = False
# lppc_model.fc_4x[0].weight.requires_grad = False
# lppc_model.fc_4x[0].bias.requires_grad = False
# lppc_model.fc_1y[0].weight.requires_grad = False
# lppc_model.fc_1y[0].bias.requires_grad = False
# lppc_model.fc_2y[0].weight.requires_grad = False
# lppc_model.fc_2y[0].bias.requires_grad = False
# lppc_model.fc_3y[0].weight.requires_grad = False
# lppc_model.fc_3y[0].bias.requires_grad = False
# lppc_model.fc_4y[0].weight.requires_grad = False
# lppc_model.fc_4y[0].bias.requires_grad = False

# Define Loss, Optimizer
loss_all = np.zeros((n_epochs,num_batches))
criterion = MSELoss()
optimizer = Adam(lppc_model.parameters(),lr=lr)
prev_loss = 0.1
...
#            # estimator output
            ...
            outputppc = lppc_model(ppc_x,ppc_y,ppc_t,1)
            output_ppc[:,t,:] = torch.reshape(outputppc,(batch,3))
            # M1
            # X
            control_x = torch.zeros((batch,1,1,3))
            control_x = torch.cat((output_ppc[:,t,0],output_ppc[:,t,2],batch_x[:,t,3],output_c[:,t-1,0],output_e[:,t-1,0],output_e[:,t,0]),0)
            # Y
            control_y = torch.zeros((batch,1,1,3))
            control_y = torch.cat((output_ppc[:,t,1],output_ppc[:,t,2],batch_x[:,t,3],output_c[:,t-1,1],output_e[:,t-1,1],output_e[:,t,1]),0)
            ...
            # spx = ((math.cos(math.radians(theta)))*op[0] - (math.sin(math.radians(theta)))*op[1])#*(1/2)            
            # spy = ((math.sin(math.radians(theta)))*op[0] + (math.cos(math.radians(theta)))*op[1])#*(1/2)
...
#        print("\nLoss: {:.8f}".format(loss.item()))
        ...
        j = j+1    
    # torch.save(lppc_model.state_dict(), "models/rppc_lppc/lppc/PM/stimulated/new_new/v18_only_dir_train_w_gf_in_all_0_75/lppc_v1_"+str(sub_num)+"_"+str(epoch)+"_epochs_v18")
    avg_loss = np.sum(loss_all[epoch-1,:])/num_batches
    print("\nLoss: ",avg_loss)#.format(loss.item()))
    if avg_loss < prev_loss:
        ## Save model
        torch.save(lppc_model.state_dict(), "models/rppc_lppc/lppc/extensions/1_normal_500")
        prev_loss = avg_loss    
...
#fname = "models/rppc_lppc/lppc/direction_adaptation_v2/losses.xlsx"
...
#plt.ylabel('Error')
#plt.savefig("models/rppc_lppc/lppc/direction_adaptation_v2/training_loss.png")
